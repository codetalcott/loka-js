...
// Spanish-localized psatina-modular: looks for `template[p:datos]` (the
// Spanish entry directive) instead of the canonical `template[p:data]`.
//
// Bridges fixi swaps and psatina-modular's auto-init: when fixi swaps in
// new HTML that contains psatina templates, this listener re-initializes
// them. Marker attribute (data-rescan) keeps it idempotent.
//
// Load order: this script (type=module) must come AFTER psatina-modular's
// es.js module in document order, so its initial "mark already-initialized
// templates" pass runs after es.js's auto-init.

import { psatina } from '../../../psatina-modular/lib/psatina.js';
